angular.module('lsi').controller('fortinentcampcreateCtrl', function ($scope, fortinetCampaignService) {
	...
	$scope.constants = {
		...
		"ALERT_DEL_ALL_MSG" : "Do you want to delete all Campaigns? This action cannot be undone and campaigns details will be lost.",
		"TRIGGER_T1" : "Once",
    ...
    $scope.fortinetNewCmpDates = { startDate: moment(), endDate: moment().add(7, 'day') };
	...
	$scope.getAllCampaigns = function () {
		fortinetCampaignService.getAllCampaigns( function (response) {
			angular.forEach(response, function (campaign) {
				if(campaign.couponUrl === null) {
					...
					var cpnId = arrVals[1];
					fortinetCampaignService.getCampaignCoupon(cpnId, function (res) {
						if(res) {
							campaign.couponUrl = "data:image/png;base64," + res;
						}
					});
				}
	...
	fortinetCampaignService.getCampaignData( function (response) {
		if(response) {
			$scope.campDetails = response;
		}
	});
	...
	$scope.setStatus = function (id) {
		angular.forEach($scope.allCampaigns, function (campaign) {
			if(id == campaign.id) {
				...
				fortinetCampaignService.setStatusCampaign( action, cmpId, trigger, function (response) {
	...
	$scope.rmvCamp = function () {
		...
		if(value === true) {
			var id = selctd_id;
			fortinetCampaignService.removeCampaign( id, function (response) {
	...
	$scope.rmvAllCmps = function () {
		...
		if(value === true) {
			fortinetCampaignService.removeAllCampaigns(function (response) {
    ...
    $scope.saveCampaign = function (d, t) {
    	$scope.checkPostParams($scope.newCampaign, function (err, res) {
    		if(err) {
		    	...
		    	//enrichData.timeToSend =  moment(t).format($scope.constants.TIME_FORMAT);
				...
				fortinetCampaignService.createCampaign( postCamp, function (response) {
					$scope.addCmp = false;
					$scope.getAllCampaigns();
				});
		    }
    ...
    $scope.editCampValues = function () {
    	...
    	$scope.fortinetNewCmpDates = { startDate: $scope.editCamp.startDateTime, endDate: $scope.editCamp.endDateTime };
    ...
    $scope.updateCampaign = function (dt, tme) {
    	...
    	fortinetCampaignService.updateCampaign(camp, function (response) {
			$scope.addCmp = false;
			$scope.update = false;
			$scope.getAllCampaigns();
		});
	...
	$scope.getAreasInfo = function (category, list) {
		...
		if(category === "Area Type") {
			...
			$scope.campDetails[key].forEach(function (element) {
				result.push({"typeId" :element.id});
			})
		} else if(category === "Area") {
			key = "areas";
			$scope.campDetails[key].forEach(function (element) {
				result.push({"areaId" :element.id});
			})
		} else if(category === "Area Interest Category") {
			key = "areaCategory";
			$scope.campDetails[key].forEach(function (element) {
				result.push({"categoryId" :element.id});
			})
		}
		return result;
	}
	...
	$scope.getStoreInfo = function (strVal) {
		...
		$scope.campDetails['stores'].forEach(function (element) {
			if(element.storeName === strVal) {
				result.push({"storeId" :element.id});
			}
		})
		return result;
	}
