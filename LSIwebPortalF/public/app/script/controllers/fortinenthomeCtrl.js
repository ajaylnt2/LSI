angular.module('lsi').controller('fortinenthomeCtrl', function ($scope, fortinetService) {
    ...
    /*datepicker starts*/
    $scope.fortinetCRDates = { startDate: moment().subtract(7, 'day'), endDate: moment() };
    $scope.fortinetFrqDates = { startDate: moment().subtract(90, 'day'), endDate: moment() };
    /*datepicker ends*/
    ...
    $scope.getCaptureRateDetails = function () {
        ...
        if (($scope.branchStatus.length == 0) && ($scope.storeStatus.length == 0) && ($scope.floorStatus.length == 0) && ($scope.accessPtStatus.length == 0)) {
            ...
            fortinetService.getCaptureRate(captureData, function (response) {
                console.log(response);
                $scope.capturedValues = response;
                $scope.graphVisible = true;
                $scope.errorCapt = false;
                $scope.loader = false;
                var getDates = [], percentIn = [], percentPer = [];
    ...
    $scope.getFrequencyDetails = function () {
       ...
       // time in miliseconds
        ...
        fortinetService.getFrequency(frequencyData,$scope.ffRecValue, function (response) {
            console.log(response);
