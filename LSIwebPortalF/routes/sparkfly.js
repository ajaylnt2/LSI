  ...
  sqlRequest.query('select * from userAuthenticationTable', function (err, recordset) {
    ...
    for(var attributename in recordset){      
      if(recordset[attributename].EmailId == req.body.emailId){
        userExist = true;
        header ={'X-Auth-Identity':recordset[attributename].XAuthIdentity,'X-Auth-Key':recordset[attributename].XAuthKey} ;
      }
    }
    if(userExist){
...
router.get('/auth', function(req, res) {
    ...
    var reqGet = http.request(optionsget, function (res1) {
        if(res1.headers['x-auth-token'] == undefined){
            res.json({status:"error",msg:"Authentication Error"});
        }else{
           var authKey = {authToken:res1.headers['x-auth-token']};
            res.json(authKey);
                ...
                if(response.statusCode == 200){
                   var dataQueue = "";    
                    ...
                    response.on("end", function () {
                        res.json({"status":"success","sData":JSON.parse(dataQueue)});
                    });
                }
            }
        );
